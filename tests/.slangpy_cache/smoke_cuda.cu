...
#line 19 "D:/git_repo/slang-python/tests/smoke.slang"
__global__ void myKernel(TensorView inValues_0, TensorView outValues_0, float v_0)
{
    uint2 _S1 = uint2{((threadIdx)).x, ((threadIdx)).y};
...
#line 21
    float _S4 = ((inValues_0).load<float>((_S2), (_S3)));
    (outValues_0).store<float>((_S2), (_S3), (0.0));
    float oldVal_0;
    float _S5 = (*((&oldVal_0)) = atomicAdd((outValues_0).data_ptr_at<float>((_S1)), (v_0 + _S4)));
    return;
}
