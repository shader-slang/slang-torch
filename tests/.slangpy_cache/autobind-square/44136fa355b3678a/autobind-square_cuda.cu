...
extern "C" {
__global__ void __kernel__square(TensorView input_0, TensorView output_0)
{
    uint _S1 = (((threadIdx)) + ((blockIdx)) * ((blockDim))).x;
    uint _S2 = ((input_0).sizes[(0U)]);
    if(_S1 >= _S2)
    {
        return;
    }
    float _S3 = ((input_0).load<float>((_S1)));
    float _S4 = ((input_0).load<float>((_S1)));
    (output_0).store<float>((_S1), (_S3 * _S4));
    return;
}
