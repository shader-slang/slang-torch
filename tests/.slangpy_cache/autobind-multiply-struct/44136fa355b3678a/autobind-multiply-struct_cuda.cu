...
extern "C" {
__global__ void __kernel__multiply(Foo_0 foo_0, TensorView result_0)
{
    uint2  location_0 = uint2 {(((blockDim)) * ((blockIdx)) + ((threadIdx))).x, (((blockDim)) * ((blockIdx)) + ((threadIdx))).y};
    uint _S1 = location_0.x;
    uint _S2 = ((foo_0.A_0).sizes[(0U)]);
    bool _S3 = _S1 < _S2;
    uint _S4 = location_0.y;
    uint _S5 = ((foo_0.B_0).sizes[(1U)]);
    if(_S3 && _S4 < _S5)
    {
        float _S6 = ((foo_0.A_0).load<float>((_S1), (_S4)));
        float _S7 = ((foo_0.B_0).load<float>((_S1), (_S4)));
        (result_0).store<float>((_S1), (_S4), (_S6 * _S7));
    }
    return;
}
