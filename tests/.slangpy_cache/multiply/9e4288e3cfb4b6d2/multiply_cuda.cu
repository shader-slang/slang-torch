...
__device__ float computeOutputValue_0(TensorView A_0, uint2  loc_0)
{
    float _S1 = ((A_0).load<float>((loc_0.x), (loc_0.y)));
    return _S1 * 2.0f;
}

extern "C" {
__global__ void mul_kernel(TensorView A_1, TensorView result_0)
{
    uint2  location_0 = uint2 {(((blockDim)) * ((blockIdx)) + ((threadIdx))).x, (((blockDim)) * ((blockIdx)) + ((threadIdx))).y};
    float _S2 = computeOutputValue_0(A_1, location_0);
    (result_0).store<float>((location_0.x), (location_0.y), (_S2));
    return;
}
